...
# Пауза внутри Whisper-сегмента при которой разбиваем на отдельные фразы для LLM.
# Меньше = LLM видит более мелкие фразы и точнее находит повторы внутри сегмента.
# Должно быть > PAUSE_CUT_SEC (иначе каждое слово станет отдельной фразой).
PHRASE_SPLIT_SEC = 1.5  # секунды
