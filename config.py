...
# Silero VAD работает на аудио-сигнале и игнорирует вздохи/шумы
VAD_MIN_SILENCE_MS = 400   # миллисекунды

# Буфер вокруг каждого сегмента (VAD добавляет его автоматически)
VAD_SPEECH_PAD_MS = 200    # миллисекунды

# ─── Буферы рендера (границы вырезанных клипов) ────────────────────────────
# Не путать с VAD_SPEECH_PAD_MS — это отдельные настройки для FFmpeg-нарезки.
# ВАЖНО: большой start-буфер → вдохи и хвосты удалённых блоков в начале фраз.
# Старая архитектура намеренно держала pre-roll = 0 именно по этой причине.

BLOCK_END_BUFFER_SEC   = 0.15  # сек после последнего слова (нужен для затухания)
BLOCK_START_BUFFER_SEC = 0.05  # сек до первого слова (минимум — только чтоб не кликало)
