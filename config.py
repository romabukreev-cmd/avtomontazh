...
# Пауза между словами длиннее этого значения → граница нового блока.
# Чем меньше — тем мельче блоки, тем точнее LLM видит повторы.
PAUSE_THRESHOLD_SEC = 0.4   # секунды

# Буферы вокруг каждого блока при рендере.
# ВАЖНО: большой start-буфер → вдохи и хвосты удалённых блоков в начале фраз.
BLOCK_START_BUFFER_SEC = 0.05  # сек до первого слова
BLOCK_END_BUFFER_SEC   = 0.15  # сек после последнего слова

# Настройки Silero VAD внутри Whisper (детекция речи)
VAD_MIN_SILENCE_MS = 400   # миллисекунды — граница между речевыми регионами
VAD_SPEECH_PAD_MS  = 200   # миллисекунды — буфер вокруг речи
