...
# ─── Границы речевых блоков ───────────────────────────────────────────────────
# Буферы вокруг каждого речевого блока (применяются и в transcriber, и в split_on_pauses)
SEG_BUF_START   = 0.05   # секунд перед первым словом
SEG_BUF_END     = 0.15   # секунд после последнего слова
MAX_WORD_DUR    = 1.5    # максимальная длительность одного слова (cap на Whisper-аномалии)

# Пауза между словами длиннее этого → разрыв (режется тишина внутри сегмента)
PAUSE_CUT_SEC = 0.5
