...
class VideoRenderer:
    ...
    def _render_single_input(
        ...
        """Рендер с одним входным файлом (без вебки)."""
        if not timeline:
            raise RuntimeError("Пустой timeline — нечего рендерить.")
        ...
        log.info(f"Рендер {output_name} (screen only): {len(timeline)} отрезков")

        chunk_paths: List[Path] = []
        try:
            for i, seg in enumerate(timeline):
                chunk_path = chunks_dir / f"chunk_{i:04d}.mp4"
                self._render_chunk_single(seg, screen_file, chunk_path, filter_builder)
                chunk_paths.append(chunk_path)
...
def _screen_only_filter(crop_y: int, s: float, e: float) -> str:
    return (
        f"[0:v]trim=start={s:.3f}:end={e:.3f},setpts=PTS-STARTPTS,"
