...
═══════════════════════════════════════════════
Между сегментами в транскрипции указана длина молчания (═══ пауза Xс ═══).
Используй это для понимания структуры:
- Длинная пауза (≥ 10с) — автор работает молча. Это граница между частями процесса.
- Короткая пауза (< 5с) — нормальный ритм речи. Повторы и переформулировки
  происходят именно здесь: одна и та же мысль в двух соседних сегментах
  с короткой паузой между ними.
...
2. ЦЕПОЧКИ ПОПЫТОК / СМЫСЛОВЫЕ ПОВТОРЫ
   Автор часто проговаривает одну и ту же мысль несколько раз подряд,
   каждый раз немного иначе:
     — первая попытка (обрывается или не звучит)
     — вторая (чуть лучше, но снова переделывает)
     — третья (почти до конца, потом останавливается)
     — финальная (совсем другими словами, но та же идея)
   Такая последовательность — это ОДНА мысль, а не четыре разные.

   Алгоритм:
   1. Для каждого сегмента сформулируй его суть в 5–7 словах.
   2. Просмотри предыдущие 10–15 сегментов и найди ВСЕ, что несут ту же суть
      (даже если слова абсолютно разные — важен смысл).
   3. Эта группа сегментов — цепочка попыток одной мысли.
      Оставь из неё ТОЛЬКО самую полную и чёткую версию (обычно последнюю),
      всем остальным ставь keep=false.

   ВАЖНО: количество попыток в цепочке не ограничено — их может быть 2, 5 или 10.
   Ищи не «пару», а всю группу семантически эквивалентных сегментов.

   Исключение: если автор возвращается к теме чтобы УГЛУБИТЬ или ДОБАВИТЬ новое —
   это развитие мысли, не повтор. Оставляй.

   Признаки фальстарта:
   - Сегмент обрывается на полуслове или явно не закончен
   - Следующий сегмент начинает ту же мысль заново (с начала или по-другому)
...
1. Пройдись по всем сегментам с keep=true.
   Для каждого — проверь, нет ли в предыдущих 10–15 сегментах других сегментов
   с той же сутью (группа попыток одной мысли может быть от 2 до 10+ штук).
   Из каждой такой группы оставь только самую полную/чёткую, остальным — keep=false.
