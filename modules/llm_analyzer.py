...
def _build_system_prompt(total_duration: float, max_sec: float) -> str:
    ...
    max_min   = max_sec / 60
    excess    = total_duration - max_sec

    if excess > 0:
        duration_block = (
            f"Исходный хронометраж: ~{total_min:.0f} мин ({total_duration:.0f}с)\n"
            f"Лимит итогового видео: {max_min:.0f} мин ({max_sec:.0f}с)\n"
            f"⚠️ ПРЕВЫШЕНИЕ: {excess:.0f}с ({excess/60:.1f} мин) — ОБЯЗАТЕЛЬНО убери.\n"
            f"Недостаточно просто пометить keep=false для повторов — этого может не хватить.\n"
            f"Удаляй менее важные сегменты пока суммарная длительность keep=true НЕ станет ≤ {max_sec:.0f}с.\n"
            f"В каждой строке сегмента указана его длительность в секундах — используй её для подсчёта."
        )
    else:
        duration_block = (
            f"Исходный хронометраж: ~{total_min:.0f} мин ({total_duration:.0f}с)\n"
            f"Лимит итогового видео: {max_min:.0f} мин ({max_sec:.0f}с)\n"
            f"Хронометраж в норме — удалять ничего не нужно (только повторы)."
        )

    return f"""Ты — видеомонтажёр для YouTube. Тебе дана полная транскрипция видео.
Сделай всё за один проход: найди повторы, оцени важность, примени лимит.
...
═══════════════════════════════════════════════
ШАГ 2: ВАЖНОСТЬ И ЛИМИТ
═══════════════════════════════════════════════
Для всех keep=true сегментов выставь score 0.0–1.0.

Если есть превышение лимита (указано выше) — обязательно переводи keep=false
для наименее важных сегментов пока суммарная длительность keep=true ≤ {max_sec:.0f}с.
Считай секунды по полю (Xс) в каждой строке.
...
- Сегменты, удаление которых оставит соседний незавершённым
  или начинающимся с середины мысли
