...
Пайплайн:
  1. concat_files()    — склеить пронумерованные части сессии (экран / вебка)
  2. transcribe()      — Deepgram → sentence-level сегменты с word timestamps
  3. analyze()         — LLM чанками → kept_segments
  4. _detect_silence() — silencedetect, страховка от длинных пауз без слов
  5. _build_timeline() — удалить паузы по word-таймкодам + тихим зонам → timeline
  6. render()          — FFmpeg → vertical_9min.mp4 или horizontal_9min.mp4
"""
...
_SILENCE_NOISE_DB = -40
_SILENCE_MIN_DUR  = 1.5
_MIN_SPEECH_CHUNK = 0.3
...
def _build_timeline(
    ...
    if silence_regions:
        ...
        for s, e in merged:
            filtered.extend(_subtract_silence(s, e, silence_regions))
        if filtered:
            after_dur = sum(e - s for s, e in filtered)
            log.info(f"Silence filter: {before_dur:.0f}с → {after_dur:.0f}с "
                     f"(вырезано {before_dur - after_dur:.0f}с)")
            merged = filtered
        else:
            log.warning("Silence filter убрал ВСЕ интервалы — оставляем без фильтра")

    padded = [
        [max(0.0, s - config.TIMELINE_START_PAD_SEC), e + config.TIMELINE_END_PAD_SEC]
        for s, e in merged
    ]
    ...
    if result:
        ...
        log.info(f"Timeline: {len(result)} интервалов, total={round(total_dur, 1)}с")
    return result
...
def _pipeline(
    ...
    if audio_source is None:
        raise RuntimeError("Нет видеофайлов для обработки.")
    done("✅ Файлы готовы")

    # 2. Транскрипция
    working("⏳ Транскрипция (Deepgram)...")
